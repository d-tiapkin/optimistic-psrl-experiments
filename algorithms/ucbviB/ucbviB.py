...
class UCBVIAgent(AgentWithSimplePolicy):
    ...
    bonus_type : {"simplified_bernstein", "hoeffding", "bernstein"}
        Type of exploration bonus If `reward_free` is true, this parameter is ignored
    ...
    def _compute_bonus(self, n, hh, var=None):
        # reward-free
        if self.reward_free:
            bonus = 1.0 / n
            return bonus
        ...
        if self.bonus_type == "simplified_bernstein":
            ...
            bonus = np.sqrt(var / n) + self.v_max[hh] / n
            bonus = min(bonus, self.v_max[hh])
            return bonus
        elif self.bonus_type == "hoeffding":
            bonus = np.sqrt(1 / n) * (self.v_max[hh] / 2)
            bonus = min(bonus, self.v_max[hh])
            return bonus
        else:
            raise ValueError(
    ...
    def _update(self, state, action, next_state, reward, hh):
        if self.stage_dependent:
            ...
            # Compute variance for Bernstein bonuses
            if hh + 1 < self.horizon:
                pVsq = np.inner(self.P_hat[hh, state, action], self.V[hh+1]**2)
                pV = np.inner(self.P_hat[hh, state, action], self.V[hh+1])
                var = np.clip(pVsq - pV**2, 0, self.v_max[hh]**2)
            else:
                var = 0

            self.B_sa[hh, state, action] = self._compute_bonus(nn, hh, var)
            ...
            # Compute variance for Bernstein bonuses
            if hh + 1 < self.horizon:
                pVsq = np.inner(self.P_hat[state, action], self.V[hh+1]**2)
                pV = np.inner(self.P_hat[state, action], self.V[hh+1])
                var = np.clip(pVsq - pV**2, 0, self.v_max[hh]**2)
            else:
                var = 0
